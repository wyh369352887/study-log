...
`callWithErrorHandling`的最后一个参数`[__DEV__ ? shallowReadonly(instance.props) : instance.props, setupContext]`其实就是`setup`的参数，与文档中`(props,context)`相对应。而在开发环境中，为了对一些错误操作做提示，对`props`包了一层proxy代理；由于`props`本身就是响应式的，这里再包一层，无需重复收集依赖。
